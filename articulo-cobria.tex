...
\input{editorial/generated/identity.tex}
\usepackage{hyperref}\hypersetup{hidelinks,pdftitle={Contratos reconstruibles con ámbito explícito para sistemas NoSQL documentales},pdfauthor={\ifnum\blindreview=1 Autor anonimizado\else\COBRIAAutor\fi},pdfsubject={Artículo científico de la propuesta independiente COBRIA},pdfkeywords={COBRIA, NoSQL documental, ámbito, reconstrucción, procedencia}}
...
\author{\ifnum\blindreview=1 Autor anonimizado\else\COBRIAAutor\\Investigador independiente, \COBRIALugar\fi}
\date{\COBRIAAnio}
...
La evidencia incluye una réplica histórica de 270 corridas sobre MongoDB 8.0,
CouchDB 3.4 y OpenSearch 2.19.3, además de una repetición preliminar del arnés corregido
sobre MongoDB y CouchDB. Las 270 corridas se conservan como evidencia histórica porque
emplearon un oráculo posteriormente fortalecido; no se presentan como certificación
vigente. El resultado principal es una especificación neutral, un conjunto instalable
de conformidad y una agenda empírica reproducible para decidir cuándo una proyección
aporta valor sin convertirse en una segunda fuente de verdad.
\end{abstract}
...
\item un arnés de conformidad reutilizable y una aplicación neutral ejecutable;
\item una separación explícita entre evidencia histórica, vigente y pendiente.
\end{enumerate}
...
\section{Resultados disponibles y estado de la evidencia}
...
La réplica distribuida histórica conservó 270 corridas: tres motores, tres escalas y
treinta repeticiones por celda. Incluyó MongoDB 8.0, CouchDB 3.4 y OpenSearch 2.19.3.
Todas las celdas superaron P1, P2, R1, S1 y A1 bajo el oráculo utilizado entonces, sin
recuperación cruzada de documentos entre ámbitos. Una auditoría posterior fortaleció
el oráculo; por ello, esas corridas describen comportamiento histórico y no conformidad
vigente. Una repetición preliminar con el arnés corregido completó casos pequeños en
MongoDB y CouchDB. OpenSearch 3.2.0 alcanzó estado saludable, pero falló durante la carga
bajo emulación; el incidente se conserva como incompatibilidad de plataforma y no como
resultado de COBRIA.

Como comparación complementaria, un experimento algorítmico controlado contrasta siete
estrategias: consulta del documento, índice nativo, vista materializada, CQRS,
reconstrucción por eventos, proyección sin contrato y proyección COBRIA. El conjunto
contiene 630 observaciones (siete estrategias, tres escalas y treinta repeticiones) y
registra p50, p95, p99, CPU, memoria, almacenamiento, recuperación y amplificación de
escritura. Su propósito es hacer visibles los costos relativos del contrato; al ejecutarse
en un único proceso, no se interpreta como comparación de rendimiento entre motores.
...
controles SQL históricos permanecen segregadas y no alimentan el resumen ni las
conclusiones.
...
El paquete de réplica incorpora además una aplicación mínima de observaciones de
biodiversidad. Se organiza en dominio, aplicación, puertos e infraestructura; exige
ámbito antes del acceso, rechaza revisiones obsoletas, aplica idempotencia ligada a la
huella de entrada y reconstruye un resumen por especie. Seis pruebas automáticas cubren
registro válido, autorización previa, repetición sin duplicado, conflicto de clave,
monotonicidad y equivalencia tras retirar y regenerar la proyección. Su función es
pedagógica y de transferencia: no sustituye los experimentos P1--A1.

\section{Trazabilidad entre hipótesis y artefactos}

\begin{table}[H]\centering
\caption{Matriz de trazabilidad del artículo.}
\begin{tabularx}{\textwidth}{p{1.7cm}p{3.1cm}p{3.2cm}X}\toprule
...
exclusiones y correspondencia entre evidencia y afirmaciones. Las cifras confirmatorias
futuras se incorporarán solo después de repetir las celdas distribuidas con el arnés
corregido y reproducir las salidas desde el depósito.
...
concretas, junto con una réplica histórica distribuida claramente delimitada. La
afirmación cuantitativa fuerte permanece abierta hasta repetir P1--A1 con el oráculo
corregido, ejecutar OpenSearch 3.2.0 en una plataforma compatible y obtener réplica
independiente. Esta delimitación transforma una intuición arquitectónica en un programa
científico refutable.
